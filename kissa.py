...
class Kissa:
    ...
    async def declareBeingThis(self, update: Update, context: CallbackContext):
    ...
    async def findWhatUserIs(self, update: Update, context: CallbackContext):
        user_id = update.message.from_user.id
        chat_id = update.message.chat.id
    ...
    async def messageHandler(self, update: Update, context: CallbackContext):
        ...
        if msg.text is not None:
            if re.match(r"oon\s(\S+\s){0,4}(tää|toi)", msg.text.lower()):
                await self.declareBeingThis(update, context)
            elif 'mikä mä oon' in msg.text.lower():
                await self.findWhatUserIs(update, context)
